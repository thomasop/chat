...
const Add = async (req: Request, res: Response) => {
    ...
    const addConversation = await Conversation.create({last_message_id: null, userOneId: userId, userTwoId: userIdAdd})
    if (addConversation === null) {
        return res.status(400).json({error: 'La conversation n\'a pas été créé'})
    } else {
        const addMessage = await Message.create({content: content, conversationId: addConversation.dataValues.id, userId: userId })
        if (addMessage === null) {
            return res.status(400).json({error: 'Le message n\'a pas été créé'})
        } else {
            const editConversation = await Conversation.update({ last_message_id: addMessage.dataValues.id }, {
                where: {
